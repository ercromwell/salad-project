...
def make_one_learner(subset_size, split, t, X, y): #subset_size is number of pairs, t is for only testing purposes
   ...
   # q = 0 #random number
    ...
    if isTree:
        ...
        clf = GradientBoostingClassifier(n_estimators= 100, max_depth= 3,subsample= 0.5, random_state= 0).fit(X, y) # for boosting
...
print "From over 0.2"
salad_defs.show_things(ensemble, Xtest1 , ytest1, isTree, False)

print "Once again, over 0.2"
Xtest2 = X_test[half:]
ytest2=y_test[half:]

salad_defs.show_things(ensemble, Xtest2 , ytest2, isTree, False)
...
Xtest2, ytest2 = salad_defs.get_pairs_under_cosine_threshold(matrix, hasColor)
salad_defs.show_things(ensemble, Xtest2 , ytest2, isTree, False)
...
#pickle it!
name = 'gtbs_100_estimators_3_depth_over_cos_threshold_v2.pi'
file_pi = open(name, 'w') 
