...
matrix = cm.add_centrality(matrix, graph, start_ingred, end_ingred)
print len(matrix[0])

recipe_pairs, y_vector, X_under, y_under = salad_defs.get_pairs_cosine_threshold(matrix, hasFeatures,num_ingred, end_ingred )
...
def make_one_learner(subset_size, split, t, X, y): #subset_size is number of pairs, t is for only testing purposes
   ...
   # q = 0 #random number
    ...
    if isTree:
        ...
        clf = GradientBoostingClassifier(n_estimators= 600, max_depth= 3,subsample= 0.5, random_state= 9).fit(X, y) # for boosting
